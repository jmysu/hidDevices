...
public:
	...
	KLed::ShapeLED shape = KLed::Circular;
...
KLed::KLed(const QColor &color, State state, Look look, ShapeLED shape, QWidget *parent)
	: QWidget(parent)
	, d(new KLedPrivate)
{
...
KLed::ShapeLED KLed::shape() const
{
	return d->shape;
}
...
void KLed::setShape(ShapeLED shape)
{
	if (d->shape == shape) {
		return;
	}
...
{
	setStyleSheet("background-color: rgba(0,0,0,0)");      //set background transparent
	if (d->color == color) {
		return;
	}
...
void KLed::paintEvent(QPaintEvent *)
{
	...
	if (d->state == On) {
		...
		QImage img(10, 1, QImage::Format_ARGB32_Premultiplied);
		QPainter p(&img);
